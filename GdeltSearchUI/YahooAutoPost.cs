...
internal enum YahooAutoPostOutcome { RecentlyPosted, Posted, MissingCredentials, Failed }
...
{
    private const double MinHoursBetweenPosts = 8.0;
    ...
    public static async Task<YahooAutoPostResult> PostIfNeededAsync(CancellationToken ct = default)
    {
        ...
        if (lastPost.HasValue && (DateTime.Now - lastPost.Value).TotalHours < MinHoursBetweenPosts)
            return new(YahooAutoPostOutcome.RecentlyPosted, LastPostedAt: lastPost.Value);
        ...
        catch (Exception ex)               { return new(YahooAutoPostOutcome.Failed, $"Caption failed: {ex.Message}"); }
    ...
    {
        var allTags     = tags.Prepend("OOTT").Distinct().ToArray();
