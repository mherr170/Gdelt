...
internal sealed record GdeltSearchResult
{
...
internal sealed class GdeltApiClient : IDisposable
{
    ...
    public async Task<GdeltSearchResult> SearchAsync(string query, int hours, string mode, CancellationToken ct, int maxRecords = 75)
    {
        ...
        var raw = await FetchWithRetryAsync(BuildUrl(query, hours, mode, maxRecords), ct);
    ...
    // Returns null on timeout after all attempts, throws on cancellation/network error.
