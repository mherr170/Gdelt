...
internal static class DebtAutoPost
{
    ...
    {
        AppLogger.Log("DebtAutoPost: fetching latest Treasury data…");
        NationalDebt data;
        using (var client = new DebtApiClient())
        {
            ...
            {
                AppLogger.Log($"DebtAutoPost: fetch exception — {ex.Message}");
        ...
        if (!data.IsSuccess)
        {
            AppLogger.Log($"DebtAutoPost: API error — {data.ErrorMessage}");
        ...
        AppLogger.Log($"DebtAutoPost: latest record date = {recordDate}");

        if (DebtPostTracker.HasBeenPosted(recordDate))
        {
            AppLogger.Log($"DebtAutoPost: already posted for {recordDate}, skipping.");
        ...
        if (creds is null)
        {
            AppLogger.Log("DebtAutoPost: no Bluesky credentials configured.");
            return new(DebtAutoPostOutcome.MissingCredentials, recordDate);
        }

        AppLogger.Log($"DebtAutoPost: posting for {recordDate} as '{creds.Value.Handle}'…");
        ...
        if (result.Ok)
        {
            AppLogger.Log($"DebtAutoPost: posted successfully for {recordDate}.");
            DebtPostTracker.MarkPosted(recordDate);
            return new(DebtAutoPostOutcome.Posted, recordDate);
        }
        AppLogger.Log($"DebtAutoPost: post failed — {result.Error}");
